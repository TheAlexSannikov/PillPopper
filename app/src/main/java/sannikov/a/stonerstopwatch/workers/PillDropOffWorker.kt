...
//        val appContext = applicationContext // this is a thing
        ...
        val drug = drugRepository.loadByDrugId(pill.drugId)
        ...
        return when (workType) {
            WorkerParams.WORK_TYPE_DROP_OFF -> {
                ...
                // update the pill
                if (updatePillAndCheckIfUpdateEnablePopping(pill, drug)) {
                    // only notify if the pill dropping off enables popping
                    showNotificationPP(drug, regularDropOff = true)
                }
                Result.success()
            }
            WorkerParams.WORK_TYPE_DROP_OFF_24H -> {
                if (deletePillAndCheckIfUpdateEnablePopping(pill, drug)) {
                    // only notify if the pill dropping off enables popping
                    showNotificationPP(drug, regularDropOff = false)
                }
                Result.success()
            }
            else -> Result.failure()
     ...
     */
    private suspend fun deletePillAndCheckIfUpdateEnablePopping(pill: Pill, drug: Drug): Boolean {
        val allowedToPopBefore = pillRepository.poppingEnabled(drug)
        pillRepository.deletePill(pill)
        return (!allowedToPopBefore && pillRepository.poppingEnabled(drug))
     ...
     */
    private suspend fun updatePillAndCheckIfUpdateEnablePopping(pill: Pill, drug: Drug): Boolean {
        val allowedToPopBefore = pillRepository.poppingEnabled(drug)
